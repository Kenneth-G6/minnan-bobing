...
一个纯单机离线的浏览器应用，按「厦门常见规则」实现**经典博饼**与**状元争**两种玩法：经典模式博全套奖品，状元争只争一个状元。
...
![Tests](https://img.shields.io/badge/tests-93%20passed-2ea44f.svg)
![Offline](https://img.shields.io/badge/offline-100%25-ff9f1c.svg)

[玩法模式](#玩法模式) · [功能特性](#功能特性) · [快速开始](#快速开始) · [玩法](#玩法) · [规则](#规则) · [技术实现](#技术实现) · [测试](#测试) · [统计实验](#统计实验)
...
| 开始界面（模式选择） | 游戏主界面 | 掷骰结果 |
| :---: | :---: | :---: |
| ![开始界面](docs/screenshots/01-start.jpg) | ![游戏主界面](docs/screenshots/02-game.jpg) | ![掷骰结果](docs/screenshots/03-roll-result.jpg) |

| 历史记录 | 结算界面 |
| :---: | :---: |
| ![历史记录](docs/screenshots/04-history.jpg) | ![结算界面](docs/screenshots/05-settlement.jpg) |

### 状元争模式

| 规则速览 | 对局界面 | 连掷等博中 |
| :---: | :---: | :---: |
| ![规则速览](docs/screenshots/06-duel-rules.jpg) | ![对局界面](docs/screenshots/07-duel-game.jpg) | ![连掷等博中](docs/screenshots/08-duel-rolling.jpg) |

| 博中定格 | 结算界面 |
| :---: | :---: |
| ![博中定格](docs/screenshots/09-duel-hit.jpg) | ![结算界面](docs/screenshots/10-duel-end.jpg) |
...
| | 经典博饼 | 状元争 |
| :--- | :--- | :--- |
| **目标** | 博全套奖品，五个普通奖发完收官 | 只争状元，一圈无人超越即夺冠 |
| **判定范围** | 13 级全表（一秀 ~ 状元插金花） | 仅四红及以上 **7 级** |
| **一次行动** | 一掷定结果 | 连掷到博出状元类为止（过程骰自动重掷） |
| **结束条件** | 五个普通奖池全为 0 | 一圈走完无人抢位 |
| **一局规模（4 人）** | 平均约 **220 掷**，与人数无关 | 平均约 **513 掷**，人越多越久 |
| **结果区** | 每掷都播报（含「已满，无奖」） | 过程只跳计数，**博中才显示名号** |

> 经典模式为默认玩法，判定链、奖池与文案与历史版本**逐字节一致**；状元争是纯增量玩法。

---

## 功能特性

- **两种玩法** —— **经典博饼**博全套奖品，**状元争**只争一个状元；模式在开始界面选定，规则各自写死，互不干扰
- **完整规则实现** —— 13 级奖项判定链、状元比较、奖池递减、轮次推进、自动结算，全部按「厦门常见规则」写死，无地方差异配置
- **自动重掷演出** —— 状元争里「连掷到博中」的过程由骰子快速翻滚 + 计数条呈现，默认 60ms/次，可切 ×1/×2/×4/直接看结果四档，单回合过程硬封顶 5 秒
...
1. 选择**玩法模式**：「经典博饼」或「状元争」（默认经典）
2. 调整**玩家人数**（2–12 人，默认 6 人），可用「−/+」按钮或滑块
3. （可选）填写玩家姓名，留空自动使用「玩家N」
4. （可选）右上角切换**音效开关**，默认开启并记住选择
5. 点击 **开始博饼**；想先看规则可展开「看看规则速览」（内容随所选模式切换：经典是 13 级全表，状元争是 7 级全表 + 回合制说明）
...
- 顶栏显示**模式徽章**与「本圈还剩 N 人待博」；右栏把奖池换成**擂主榜**（擂主卡片 + 各人最佳成绩与累计掷骰数），下方为**回合记录**
- 当前玩家点击 **博一次**，骰子开始快速翻滚，碗边计数条「连掷中 N 次」快速跳动；**过程不刷骰面、不播「未中」**
- 默认单次间隔 60ms，可随时切换 **×1 / ×2 / ×4 / 直接看结果** 四档；单回合过程硬封顶 5 秒，超出自动加速
- 博中时定格为完整落碗动画 + 金效 + 音效，显示**名号、组合说明**，以及「成为擂主 / 抢下擂台」结果
- 一圈走完无人抢位即收官，收官文案「本圈无人抢位，状元定格」

### 结算界面

- 状元加冕动画（皇冠落下 + 金粉飘落 + 欢庆音效）
- **经典**：「团圆战果」逐位列出奖品清单与数量，状元卡片优先排在最前
- **状元争**：「战果」区改为**各人最佳成绩榜**（按成绩降序），并标注每人累计回合数与掷骰数；擂主领走唯一一份状元饼
- **重新开始** 回到开始界面；**再来一局（相同玩家）** 沿用名单与模式直接开局

---

## 规则

> 以下为写死的默认版本，从高到低匹配，**命中最高奖项后立即停止**。两种模式共用同一张判定表，区别在于**判定的级别范围**与**计奖方式**：经典模式使用全部 13 级，状元争只认**四红及以上**的 7 级。
...
规则同样写死在核心层，与经典模式共用 `judgeRoll` 与状元比较器，**不改动任何经典判定**：

- **判定范围**：只认**四红及以上 7 级**（四红 / 五子 / 五王 / 六勃黑 / 遍地锦 / 六杯红 / 状元插金花），单次命中率约 **1.2024%**
- **回合制**：轮到某人即「连掷到博出状元类为止」，中间的骰**自动重掷**，因此**不存在「无人博中」**，每个回合必然有结果
- **擂主制**：第一个博出状元类的人成为擂主；后续**更大的抢位成功、更小或相等则守擂成功**（比较规则与经典状元逐字一致）
- **第 1 圈**全员各博一次；**第 2 圈起擂主不再掷骰，只守擂**，其余人依次挑战
- **收官**：一圈走完**无人抢位**即结束（含第 1 圈），擂主获得唯一一份状元饼，其余玩家无奖品但保留各自最佳成绩；收官文案「本圈无人抢位，状元定格」
- **无「未中」概念**：过程掷骰不进历史、不发奖，历史按**回合**而非「每一掷」记录

> **量级与经典相反**：状元争一局的掷骰数与人数强相关（人越多回合越多）。真实引擎实测（每种人数 3 万局）：4 人平均 **513 掷**、6 人 **802 掷**、12 人 **1647 掷**；单回合重掷次数长尾明显（4 人局 p99 达 **375 次**），故演出层对单回合做了 5 秒硬封顶。
...
└── src/
    ...
    ├── core/                    # ★ 纯逻辑层，零 UI 依赖
    │   ├── types.ts             # 类型、模式元数据、奖项元数据、奖池常量
    │   ├── rules.ts             # judgeRoll() 判定 + compareRolls() 比较（两种模式共用）
    │   ├── random.ts            # 可注入随机源 + 测试用确定性随机源
    │   ├── engine.ts            # 状态机：发奖 / 状元更替 / 轮次 / 结算 / 状元争回合
    │   └── __tests__/
    │       ├── rules.test.ts        # 判定表与比较器（37 项）
    │       ├── engine.test.ts       # 经典状态机与不变量（26 项）
    │       ├── zhuangyuan.test.ts   # 状元争模式规则（17 项）
    │       └── app.smoke.test.tsx   # 界面端到端冒烟（13 项）
    └── components/
        ├── StartScreen.tsx / GameScreen.tsx / EndScreen.tsx
        ├── DiceBowl.tsx / Die.tsx           # 博饼碗与骰子
        ├── DuelPlayer.tsx                   # 状元争：自动重掷演出播放器
        ├── Leaderboard.tsx                  # 状元争：擂主榜与最佳成绩排行
...
// 比较两次掷骰（用于状元/擂主更替）：> 0 表示 a 更大，0 表示完全相等
compareRolls(a: RollResult, b: RollResult): number

// 创建一局：mode 省略时默认 'classic'，行为与历史版本逐字节一致
createGame(names: string[], mode?: GameMode): GameState

// 经典模式：执行一掷（返回新状态，不可变更新）
rollOnce(state: GameState, random?: RandomFn): { state: GameState; outcome: RollOutcome }

// 状元争模式：执行一个完整回合（连掷到博出状元类为止）
takeTurn(state: GameState, random?: RandomFn): {
  state: GameState;
  outcome: TurnOutcome;
  sequence: number[][];   // 过程骰面序列，仅供动画播放，不写入 state
}

// 结束判定按模式分派：经典 = 普通奖池全空；状元争 = 本圈博完且无人抢位
isFinished(state: GameState): boolean
```

`rollOnce` 与 `takeTurn` 各自只对所属模式有效，跨模式调用会直接抛错（fail fast）。随机源可注入，测试用 `diceSequenceRandom([[4, 4, 4, 4, 1, 1]])` 构造确定性骰子序列。
...
| `zhuangyuan.test.ts`（17 项） | 回合必然以状元类收尾、过程掷骰不入历史、普通奖池全程不变、抢位 / 守擂判定（更大替换、相等或更小保留）、`challengersLeft` 抢位重置与逐圈递减、第 1 圈 N 人第 2 圈起 N−1 人、擂主回合数不再增长、确定性序列驱动的 `rolls` / `sequence` 长度、安全上限抛错、跨模式调用抛错、结算与 `Player.best` 更新、2 / 12 人整局不变量、`isFinished` 分派、经典模式回归 |
| `app.smoke.test.tsx`（13 项） | jsdom 下真实挂载渲染，走完「开始 → 掷骰 → 结算 → 重开」全流程；经典分支注入确定性骰子序列（第 1 掷插金花，其后 62 掷恰好清空五个普通奖池），状元争分支注入确定性回合序列（四回合收官），并断言开局「本圈还剩 N 人待博」、模式徽章与擂主榜渲染、轮次只累加不封顶、全程无 React 运行时错误 |

---

## 统计实验

> **口径说明**：本章数据（4 人 × 100 局、1 万局复核、223 掷等）全部属于**经典模式**。状元争模式的量级是另一套（4 人平均 513 掷、6 人 802 掷、单回合重掷 p99 375 次），已在该玩法章节说明，本章不重复统计。
...
```
dist/assets/index-*.js      190 KB（gzip  61 KB）
dist/assets/index-*.css      36 KB（gzip   8 KB）
